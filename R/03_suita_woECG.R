# Suita score calculation without ECG for data frame
suita<- function(data) {
  required_cols <- c("age", "sex", "sbp", "dbp", "t2dm", "tc", "hdl", "ldl", "urineprotein", "smoking")
  ...
  data$t2dm_score <- ifelse(data$t2dm == 1, 7, 0)
                               ...
                               ifelse(data$age >= 50 & data$age < 60, 14,
                                      ifelse(data$age >= 60 & data$age < 65, 18,
                                             ifelse(data$age >= 65 & data$age < 70, 22,
                                                    ifelse(data$age >= 70 & data$age < 75, 26,
                                                           ifelse(data$age >= 75 & data$age < 80, 29, 29)))))))

  # Total score
  data$totalscore <- data$sex_score + data$bp_score + data$hdl_score + data$cho_score +
    data$urineprotein_score + data$t2dm_score + data$smoking_score + data$age_rank
